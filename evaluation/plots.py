...
from data import denormalise, all_fcst_fields
...
def plot_img_log_coastlines(
    img, value_range_precip=(0.01, 5), cmap="viridis", extent=None, alpha=0.8
):
    plt.imshow(
        np.maximum(img, 1e-6),
        interpolation="nearest",
...
def plot_sequences(
    ...
    for kk in range(num_cases):
        ...
        #                (1.25*cond.shape[-3])-1),tf.math.floor(1.25*cond.shape[-2])])
        const = inputs["hi_res_inputs"]
        seq_real = np.squeeze(outputs["output"])
        ...
        if mode == "GAN":
            ...
            for ii in range(ens_size):
                ...
                noise_gen = NoiseGenerator(noise_shape, batch_size=batch_size)
                seq_gen.append(
                    gen.decoder.predict([mean, logvar, noise_gen(), const], verbose=0)
                )

        seq_real = denormalise(seq_real)
        seq_gen = [denormalise(seq) for seq in seq_gen]
        #print(seq_gen[0].shape)

        # plot truth
        plt.subplot(gs[kk, 0])
        plot_img(seq_real, value_range=value_range)
    ...
    if out_fn is not None:
        plt.savefig(out_fn+"_"+str(checkpoint)+".pdf", bbox_inches="tight")
        plt.close()
...
def plot_rank_histogram_all(
    ...
    if lead_time is not None:
        if threshold:
            plt.savefig(
                os.path.join(
                    log_path, f"rank-distribution-lead_time-{model}-{freq}.pdf"
                ),
                bbox_inches="tight",
            )
        else:
            plt.savefig(
                os.path.join(log_path, f"rank-distribution-lead_time-{model}.pdf"),
                bbox_inches="tight",
            )
    elif ablation:
        if threshold:
            plt.savefig(
                os.path.join(log_path, f"rank-distribution-ablation-{freq}.pdf"),
                bbox_inches="tight",
            )
        else:
            plt.savefig(
                os.path.join(log_path, "rank-distribution-ablation.pdf"),
                bbox_inches="tight",
            )
    else:
        if threshold:
            plt.savefig(
                os.path.join(log_path, f"rank-distribution-{freq}.pdf"),
                bbox_inches="tight",
            )
        else:
            plt.savefig(
                os.path.join(log_path, "rank-distribution.pdf"), bbox_inches="tight"
            )
    plt.close()
...
def plot_histograms(log_folder, val_year, ranks, N_ranks):
    rank_metrics_files = [
        os.path.join(log_folder, f"ranksnew-{val_year}-{rk}.npz") for rk in ranks
    ]
    labels = [f"{rk}" for rk in ranks]
    plot_rank_histogram_all(rank_metrics_files, labels, log_folder, N_ranks=N_ranks)


def gridplot(models, model_labels=None, vmin=0, vmax=1):
    nx = models[0].shape[0]
    ny = len(models)
...
def plot_roc_curves(roc_files, labels, log_path, precip_values, pooling_methods, lw=2):
    roc_data = {}
    ...
    for method in pooling_methods:
        for i in range(len(precip_values)):
            ...
            plt.plot(
                ...
                [],
                " ",
                label="event frequency %0.3f" % roc_data["GAN_base"][method][i],
            )  # plot base rate
            plt.xlim([0.0, 1.0])
            plt.ylim([0.0, 1.05])
            ...
            plt.savefig(
                os.path.join(log_path, f"ROC-{precip_values[i]}-{method}.pdf"),
                bbox_inches="tight",
            )
            plt.close()


def plot_prc_curves(prc_files, labels, log_path, precip_values, pooling_methods, lw=2):
    prc_data = {}
    ...
    for method in pooling_methods:
        for i in range(len(precip_values)):
            ...
            plt.plot(
                ...
                lw=0.5,
                color="gray",
                label="event frequency %0.3f" % prc_data["GAN_base"][method][i],
            )  # no skill
            plt.xlim([0.0, 1.0])
            plt.ylim([0.0, 1.05])
            ...
            plt.legend()
            plt.savefig(
                os.path.join(log_path, f"PR-{precip_values[i]}-{method}.pdf"),
                bbox_inches="tight",
            )
            plt.close()
...
def plot_fss(
    ...
    for i in range(len(precip_values)):
        ...
        for fn, label, color in zip(fss_files, labels, colors):
            ...
            plt.semilogx(
                spatial_scales, y1, "-", color=color, lw=lw, label=f"{labels[j]}"
            )
        ...
        plt.semilogx(
            [1.0, spatial_scales[-1]], [baserate, baserate], "-", color="0.9", lw=lw
        )
        plt.semilogx(
            [1.0, spatial_scales[-1]],
...
def plot_rapsd(rapsd_files, num_samples, labels, log_path, spatial_scales):
    rapsd_data = {}
            ...
            for i, colour in zip(range(len(rapsd_data_labels)), colours):
                if rapsd_data_labels[i] == "IFS":
                    ...
                    if rapsd_data_labels[i] == "TRUTH":
                        ...
                        lw = 1.0
                    R_1, freq_1 = rapsd(
                        rapsd_data[model][k][rapsd_data_labels[i]],
                        fft_method=np.fft,
                        return_freq=True,
                    )
...
def plot_preds(
        ...
        for i in range(num_rows):
            ...
            if i < (len(preds_to_plot)):
                plot_img_log_coastlines(
                    pred_data[label][k][preds_to_plot[i]],
            ...
            if i == 0:
                title = (
                    pred_data[label][k]["dates"][:4]
                    + "-"
                    + pred_data[label][k]["dates"][4:6]
                    + "-"
                    + pred_data[label][k]["dates"][6:8]
                    + " "
                    + str(pred_data[label][k]["hours"])
                    + "Z"
                )
            ...
            if k == 0:
                if i < (len(preds_to_plot)):
                    ax.set_ylabel(
                        preds_to_plot[i], fontsize=8
                    )  # cartopy takes over the xlabel and ylabel
    ...
    plt.savefig(
        os.path.join(log_path, f"predictions-{num_samples}.png"), bbox_inches="tight"
    )
    plt.close()
...
def plot_comparison(
    ...
    for k in range(num_samples):
        for i in range(num_rows):
            ...
            if i == 0:  # plot IFS
                ...
                title = (
                    pred_data["4x"][k]["dates"][:4]
                    + "-"
                    + pred_data["4x"][k]["dates"][4:6]
                    + "-"
                    + pred_data["4x"][k]["dates"][6:8]
                    + " "
                    + str(pred_data["4x"][k]["hours"])
                    + "Z"
                )
                plt.title(title, fontsize=9)
            if i == 1:  # plot TRUTH
            ...
            if i > 1:  # plot different flavours of prediction
                plot_img_log_coastlines(
                    pred_data[comp_to_plot[i]][k]["GAN pred 1"],
                    value_range_precip=value_range_precip,
                    cmap=cmap,
                    extent=extent,
                    alpha=alpha,
    ...
    # cannot save as pdf - will produce artefacts
    plt.savefig(
        os.path.join(log_path, f"comparison-{num_samples}.png"), bbox_inches="tight"
    )
    plt.close()
