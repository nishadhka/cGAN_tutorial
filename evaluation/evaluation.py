...
import sys 
sys.path.insert(1,"../")
from data import all_fcst_fields, denormalise
from config import read_downscaling_factor
from model import NoiseGenerator, pool
import setupdata
import setupmodel
...
def eval_one_chkpt(
    *,
    ...
    for kk in range(num_images):
        ...
        if denormalise_data:
            masked_truth = denormalise(masked_truth)
...
def evaluate_multiple_checkpoints(
    *,
    ...
    log_line(
        log_fname,
        f"Data initial dates/time indices: {data_gen_valid.dates[0:4]}, {data_gen_valid.time_idxs[0:4]},gen seed {data_gen_valid.seed}"
    )
    log_line(
        log_fname,
