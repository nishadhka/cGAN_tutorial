...
from evaluation import evaluate_multiple_checkpoints, plot_histograms
from config import set_gpu_mode, read_downscaling_factor
...
if __name__ == "__main__":
    ...
    df_dict = read_downscaling_factor()  # read downscaling params
    ...
    if problem_type == "normal":
        autocoarsen = False
        input_channels = 5 * len(all_fcst_fields)
    ...
    if args.do_training:
        ...
        if args.tune:
            ...
            log_file = os.path.join(log_folder, "log_tune.txt")
            log_list = []
        
        elif args.restart:  # load weights and run status
            model.load(model.filenames_from_root(model_weights_root))
        ...
        while training_samples < num_samples:  # main training loop
            ...
            if args.tune:
                with open(os.path.join(log_folder, "run_status_tune.json"), "w") as f:
    ...
    if args.plot_ranks:
        plot_histograms(log_folder, val_years, ranks=ranks_to_save, N_ranks=11)
