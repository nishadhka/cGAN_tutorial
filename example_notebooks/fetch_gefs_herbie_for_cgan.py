...
After subsetting to East Africa (ICPAC region), Herbie's cached GRIB files
are deleted to save disk space.

Usage:
    uv run fetch_gefs_herbie_for_cgan.py --date 20240520 --max-members 30
    uv run fetch_gefs_herbie_for_cgan.py --date 20240520 --output-dir herbie_cgan_output
"""
...
def setup_logging(out_dir: Path, log_path: Path) -> logging.Logger:
    ...
    # Clear any previous handlers (important when called from test script)
...
def cleanup_herbie_cache(logger=None):
    """Remove Herbie's cached GRIB files to save disk space."""
    ...
    total_freed = 0
    for d in herbie_dirs:
        if d.exists():
            # Find and remove GRIB files
            for pattern in ["**/*.grib2", "**/*.grib2.*.idx", "**/*.idx"]:
                for f in d.glob(pattern):
                    try:
                        ...
                        pass
    if total_freed > 0:
        log(f"  Cleaned up Herbie cache: freed {total_freed / (1024*1024):.1f} MB")


def fetch_single_field_herbie(
    date_str: str, run: str, fxx: int, member: int, search_pattern: str,
) -> Optional[np.ndarray]:
...
def fetch_all_variables_for_date(
    ...
    Returns:
        saved_files: dict of variable name -> output file path
        timing: dict of variable name -> seconds taken
    """
    from herbie import Herbie
    ...
    t_grid = time.time()
    test_H = Herbie(f"{date_str} {run}:00", model="gefs", product="atmos.25",
            ...
            for s_idx, fxx in enumerate(forecast_hours):
                ...
                if arr is not None:
                    if arr.shape == (len(icpac_lats), len(icpac_lons)):
                        ...
                        log_warn(f"    Shape mismatch {var_name} m{member} T+{fxx}h: "
                                 f"{arr.shape} vs ({len(icpac_lats)}, {len(icpac_lons)})")
                else:
                    log_warn(f"    Failed: {var_name} member {member} T+{fxx}h")

        var_elapsed = time.time() - var_start
        timing[var_name] = var_elapsed
        log(f"    Fetched: {success_count}/{total_fetches} in {var_elapsed:.1f}s "
            f"({var_elapsed/max(success_count,1):.2f}s per fetch)")
    ...
    # Clean up cached GRIB files — only NetCDFs with East Africa subset are kept
    log("\n  Cleaning up Herbie GRIB cache...")
    cleanup_herbie_cache(logger)

    return saved_files, timing
...
def main():
    ...
    log_file = output_dir / "herbie_fetch_log.txt"
    ...
    logger.info(f"ICPAC region: lat [{LAT_MIN}, {LAT_MAX}], lon [{LON_MIN}, {LON_MAX}]")
    logger.info(f"Output: {output_dir}")
    logger.info("")

    t0 = time.time()
    saved, timing = fetch_all_variables_for_date(
    ...
    logger.info("=" * 70)
    logger.info(f"{'Variable':<10} {'Time (s)':<12} {'Time (min)':<12}")
    logger.info("-" * 34)
    for var, secs in timing.items():
        logger.info(f"{var:<10} {secs:<12.1f} {secs/60:<12.1f}")
    logger.info("-" * 34)
    logger.info(f"{'TOTAL':<10} {elapsed:<12.1f} {elapsed/60:<12.1f}")
    logger.info("")
    logger.info(f"Variables saved: {len(saved)}/{len(CGAN_VARIABLES)}")
    total_size = sum(p.stat().st_size for p in saved.values()) / (1024 * 1024)
    logger.info(f"Total NetCDF size: {total_size:.1f} MB (East Africa only)")
