...
# print(data_paths["TFRecords"]["tfrecords_path"])
...
def write_data(
    ...
    img_chunk_width=DEFAULT_CON_SHAPE[0],  # controls size of subsampled image
    num_class=CLASSES,
    log_precip=True,
    fcst_norm=True,
    consolidated=False,
    ...
    for time_idx in np.arange(30,54,6):

        print(f"Doing time index {time_idx}")
        s_hour = time_idx #* HOURS
        e_hour = time_idx + HOURS
        if consolidated:
            dates = [str(date).replace("-","")\
             for date in\
             np.arange("%i-01-01"%year,"%i-05-01"%(year),np.timedelta64(1,"D"),dtype="datetime64[D]")\
            ]
        else:
