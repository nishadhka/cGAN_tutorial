...
def set_gpu_mode():
    ...
    if lc['use_gpu']:
        ...
        # XLA JIT: fuses the many small convs in the generator/discriminator.
        # Measured 1.27x faster per training iteration (3.63 -> 2.85 s/iter at
        # gen 64 / disc 256, batch 8) for no capacity change. It does re-order
        # float ops, so results are not bit-identical to a non-JIT run -- hence
        # opt-in via local_config.yaml rather than on by default.
        if lc.get('use_xla', False):
