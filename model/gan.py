...
import sys
sys.path.insert(1,"../")
from evaluation.wloss import wasserstein_loss, CL_chooser
...
class WGANGP(object):
    ...
    def load(self, load_files):
        ...
        with Nontrainable(self.gen):
            ...
            load_opt_weights(self.disc_trainer, load_files["disc_opt_weights"])
    ...
    def build_wgan_gp(self):
        ...
        with Nontrainable(self.disc):
            if self.mode == "GAN":
                ...
                if self.ensemble_size is not None:
                    ...
                    preds = [
                        self.gen([gen_in[0], gen_in[1], gen_in[3 + ii]])
                        for ii in range(self.ensemble_size)
                    ]
                    preds = tf.stack(preds)
                    full_gen_out.append(preds)
                self.gen_trainer = Model(
                    inputs=gen_in, outputs=full_gen_out, name="gen_trainer"
                )
            elif self.mode == "VAEGAN":
                self.gen_trainer = VAE_trainer(
                    self.gen,
                    self.disc,
                    self.kl_weight,
                    self.ensemble_size,
        ...
        with Nontrainable(self.gen):
            ...
            disc_in_avg = RandomWeightedAverage()([disc_in_real, disc_in_fake])#keras.layers.Average()
            ...
            #    target = disc_out_real
            #    wrt = disc_out_fake

            #disc_gp = tape.gradient(target, wrt)
            
            self.disc_trainer = Model(
                inputs=cond_in + const_in + noise_in + sample_in,
    ...
    def compile(self, opt_disc=None, opt_gen=None):
        ...
        if opt_disc is None:
            opt_disc = Adam(
                learning_rate=self.learning_rate_disc, beta_1=0.0, beta_2=0.999
            )
        self.opt_disc = opt_disc
        if opt_gen is None:
            opt_gen = Adam(
                learning_rate=self.learning_rate_gen, beta_1=0.0, beta_2=0.999
            )
        ...
        with Nontrainable(self.disc):
            if self.mode == "GAN":
                ...
                self.gen_trainer.compile(
                    loss=losses, loss_weights=loss_weights, optimizer=self.opt_gen
                )
            elif self.mode == "VAEGAN":
        ...
        for kk in range(num_gen_batches):
            ...
            with Nontrainable(self.disc):
                ...
                if self.ensemble_size is None:
                    ...
                    noise_list = [noise_gen() for ii in range(self.ensemble_size + 1)]
                    gt_outputs = [gen_target, sample]
                gt_inputs = condconst + noise_list

                if self.mode == "GAN":
                    gen_loss = self.gen_trainer.train_on_batch(gt_inputs, gt_outputs)
                elif self.mode == "VAEGAN":
                    gen_loss = self.gen_trainer.train_step([gt_inputs, gt_outputs])
            ...
            if self.mode == "det":
                raise RuntimeError(
                    "Doctor, what are you doing here? You're supposed to be on Gallifrey"
                )
            elif self.mode == "GAN":
