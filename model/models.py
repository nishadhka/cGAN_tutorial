...
def generator(
    ...
    if mode == "VAEGAN":
        ...
        encoder_model = Model(
            inputs=[generator_input, const_input],
            outputs=[means, logvars],
            name="encoder",
        )
        ...
        generator_output = (
            tf.multiply(noise_input, tf.exp(logvar_input * 0.5)) + mean_input
        )
    ...
    if mode == "VAEGAN":
        decoder_model = Model(
            inputs=[mean_input, logvar_input, noise_input, const_input],
        ...
        model = Model(
            inputs=[generator_input, const_input], outputs=generator_output, name="gen"
        )
        return model
